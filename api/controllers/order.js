...
module.exports = () => {
    ...
    controller.create = async (req, res) => {
        ...
        // Check Client
        let validClient = await Account.findById(client).catch(err => { return err });
        if (!validClient) {
        ...
        // Check Promotion
        if (promotion) {
            let validPromotion = await Promotion.findOne({ code: promotion }).catch(err => { return err });
            if (!validPromotion) {
        ...
        let products = [], orderTotal = 0;
        if (items !== undefined && items.length > 0) {
            for (var i = 0; i < items.length; i++) {

                // Check valid Photo
                let validPhoto = await Photo.findById(items[i].item).catch(err => { return err });
                if (!validPhoto) {
                    res.status(400).send({ 
                ...
                // Check valid Size
                let validSize = await PhotoSize.findById(items[i].size).catch(err => { return err });
                if (!validSize) {
                    res.status(400).send({ 
        ...
        newOrder.orderCount = await Order.estimatedDocumentCount({}, function(err, result) {
            if (err) {
              console.log(err);
            } else {
              return result;
    ...
    controller.stats_admin = async (req, res) => {
        ...
        orders.forEach(order => {
            sumSales += order.totalNoPromotion - (order.promotion 
        ...
        for (var sizeKey in requestedSizes) {
            if (sizeKey === 'length' || !requestedSizes.hasOwnProperty(sizeKey)) continue;
            var quantity = requestedSizes[sizeKey];

            // sizeKey => nome tamanho pedido
            // quantity => quantidade tamanho pedido
            
            let matchedSize = simpleSizes[sizeKey], priceToPay = 0;
    ...
    controller.update = async (req, res) => {
        ...
        if (req.body.remProducts === undefined && 
            ...
            if (req.body.addProducts instanceof Array) {
                for (const newProduct of req.body.addProducts ){
                    ...
                    let newProductFound = await Photo.find({ _id: newProduct.imageID })
                                                .then(data => { if (data == null) throw 'Photo Not found. '+newProduct.imageID; return data; })
                                                .catch(err => {
                                                    res.status(500).send({
                                                        message: "Error updating Order with id=" + id,
                                                        debug: err
                    ...
                    let newSizeFound = await PhotoSize.find({ _id: newProduct.sizeID })
                                                        .then(data => { if (data == null) throw 'Size Not found. '+newProduct.sizeID; return data; })
                                                        .catch(err => {
                                                            res.status(500).send({
                                                                message: "Error updating Order with id=" + id,
                                                                debug: err
                    ...
                    // add images to order
                    currentOrder.addProduct({ item: newProductFound._id, size: newSizeFound._id })
                }
            } else {
                res.status(500).send({
                    message: "Error updating Order with id=" + id,
        ...
        if (req.body.promotionCode) {
            let newPromo = await Promotion.findOne({ code: req.body.promotionCode })
                                            .then(data => { if (data.length === 0) throw 'Not found!'; return data; })
                                            .catch(err => {
                                                res.status(500).send({
